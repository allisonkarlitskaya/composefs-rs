...
impl<ObjectID: FsVerityHashValue> Downloader<ObjectID> {
    async fn fetch(&self, dir: &str, name: &str) -> Result<Bytes> {
    ...
    async fn ensure_object(&self, id: &ObjectID) -> Result<bool> {
        if self.repo.open_object(id).is_err() {
            let data = self.fetch("objects/", &id.to_object_pathname()).await?;
    ...
    async fn ensure_stream(self: &Arc<Self>, name: &str) -> Result<(Sha256Digest, ObjectID)> {
        // TODO: ideally get the ID without downloading it
        //  - etags?
        //  - HEAD for some symlink reading?
        //  - serve the symlink as a text file?
        // ...but this going to be a container config json, so it's not exactly big
        ...
        let data = self.fetch("streams/", name).await?;
        let my_id = self.repo.ensure_object(&data)?;
        progress.inc(1);
