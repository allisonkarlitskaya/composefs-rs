 ...
 *
 * NB: This format is documented in `docs/splitstream.md`.  Please keep the docs up to date!!
 */
...
fn read_range(file: &mut File, range: FileRange) -> Result<Vec<u8>> {
    let size: usize = (range.end.get() - range.start.get()).try_into()?;
...
fn take_range(mut file: File, range: FileRange) -> Result<Take<File>> {
    file.seek(SeekFrom::Start(range.start.get()))?;
    Ok(file.take(range.end.get() - range.start.get()))
}
...
impl<ObjectID: FsVerityHashValue> SplitStreamWriter<ObjectID> {
    ...
    /// Add an externally-referenced stream.
    ///
    /// This establishes a link to another stream from this stream.  The link is given a unique
    /// index number, which is returned.  Once assigned, this index won't change.  The same index
    /// can be used to find the linked stream when reading the file back.
    ///
    /// This link is considered when performing garbage collection: the linked stream will be kept
    /// alive by this stream.
    pub fn add_stream_ref(&mut self, verity: &ObjectID) -> usize {
        self.stream_refs.ensure(verity)
    }
    ...
    pub fn add_named_stream_ref(&mut self, name: &str, verity: &ObjectID) {
        let idx = self.add_stream_ref(verity);
    ...
    pub fn write_external(&mut self, data: &[u8]) -> Result<()> {
        self.total_size += data.len() as u64;
        self.write_reference(self.repo.ensure_object(data)?)
    }
    ...
    pub async fn write_external_async(&mut self, data: Vec<u8>) -> Result<()> {
        self.total_size += data.len() as u64;
        self.write_reference(self.repo.ensure_object_async(data).await?)
    }
...
impl<ObjectID: FsVerityHashValue> SplitStreamReader<ObjectID> {
    ...
    pub fn new(mut file: File, expected_content_type: Option<u64>) -> Result<Self> {
        ...
        let named_refs = read_named_references(&named_refs_bytes, stream_refs)
            .map_err(|e| Error::msg(format!("Error reading splitstream mappings: {e:?}")))?;

        let decoder = Decoder::new(take_range(file, info.stream)?)?;
